...
- Supersedes: none
- Superseded by: none
- Related: [产品需求](../requirements.md)、[系统设计](../system-design.md)、[MinerU 注意事项](../../notes/mineru.md)

## 背景

产品需要把受支持文献资产解析为轻结构化文本。解析器是满足该需求的外部技术能力，不是产品需求的一部分，也不应决定文献数据库的通用文本合同。
...
4. Parser adapter 负责把已验收资产提交给外部能力，并把返回结果转换成 SciRetriever 的 parser-neutral 轻结构化文本合同。
5. 外部任务 ID 只能作为一次解析尝试的恢复和诊断信息，不能成为文献状态、产品导航或通用后台任务模型。
6. MinerU 返回的 URL、归档、JSON、文本、图片和坐标均是不可信外部输入。只有经过协议、资源、路径、schema、页码、位置和内容边界验证的结果才能发布。
7. 发布的解析结果必须记录输入资产 hash、parser/service identity、模型或后端身份和必要参数，并能建立回到原始资产的 source locator。
8. 解析完成只表示产生了可用轻结构化文本。任何额外 LLM 分析、canonical metadata promotion、标签或领域抽取属于解析之后的独立能力，不由 MinerU 拥有。
...
- 未来可以增加 XML、HTML 或其它格式的 parser adapter，只要它们产生同一类通用轻结构化结果。
