...
| [ADR 0017：无状态共享 Agents 与受控 Browser Controller](0017-shared-agents-and-controlled-browser-agent.md) | Accepted，按 ADR 0019/0021/0023 修订 | Provider-neutral 无状态单次 Agents Runtime、独立 Analysis/Browser role、消费模块 controller 所有权、统一 Browser Observation、六种封闭动作、execute/settle、Candidate 生命周期、stable transition graph 与 controller safety fuse | Model Provider/Model 配置所有权、具体 SDK/transport、PDF 文章归属、任意 Browser/CDP 接管、长期 Agent memory |
...
| [ADR 0023：通用 Browser Agent 执行器](0023-generic-browser-agent-executor.md) | Accepted | 唯一 `browser:generic` Agent 路线、稳定 Observation/原子动作交接、Publisher profile 与 Browser 策略解耦、Browser PDF 文章归属门、单一 `browser-generic` policy 和旧规则破坏性删除 | Public/API 路线顺序、CloakBrowser binary 安装、Model Provider 配置、真实站点成功率 |
...
3. 讨论系统架构、整体数据流和模块责任时读[设计文档](../design.md)。当前 ADR 的主题为：
   ...
   - 唯一通用 Browser Agent、稳定页面交接、动态文章起点与 PDF 文章归属验收：ADR 0023。
