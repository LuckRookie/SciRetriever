...
| [ADR 0005：DocumentPackage 2.0 不兼容合同](0005-document-package-2-breaking-contract.md) | Accepted | Package 2.0 递归 schema、canonicalization、hash/URN、时间、重放、旧来源和 v1 拒绝 | CLI、JSON envelope、锁、catalog bootstrap、SDK 协议、实施进度 |
...
3. 讨论系统架构、整体数据流和模块责任时读[设计文档](../design.md)。当前 ADR 的主题为：
   ...
   - DocumentPackage 2.0 格式、身份和不兼容边界：ADR 0005。
