...
| `src/sciretriever/infrastructure/storage/` | [架构原则](../architecture/principles.md)、[设计文档](../architecture/design.md)、[技术文档](../architecture/technical.md) | 不可变内容寻址存储、hash、相对路径、权限、create-if-absent 发布与对账；SQLite schema manifest/fingerprint、fresh-only bootstrap、只读验证、短事务边界和描述符安全文件系统原语；公开切换前不得写成当前 CLI 行为 |
| `src/sciretriever/infrastructure/locking/` | [设计文档](../architecture/design.md)、[技术文档](../architecture/technical.md) | canonical catalog/output path 绑定、本机非阻塞 advisory lock、锁顺序、进程退出或崩溃释放，以及与短事务和文件对账的协调；公开切换前不得写成当前 CLI 行为 |
