...
| `src/sciretriever/discovery/`、`src/sciretriever/integrations/` | [系统设计](../architecture/system-design.md)、[技术架构](../architecture/technical-architecture.md)、[README](../../README.md)、[Provider 注意事项](../notes/providers.md) | 领域条件映射、多来源查询、候选归一化、身份收敛、默认 limit、已发布入口和外部契约；只有用户工作流或产品结果变化时同步需求 |
| `src/sciretriever/integrations/graph.py`、引用解析与 expansion | [系统设计](../architecture/system-design.md)、[技术架构](../architecture/technical-architecture.md)、[README](../../README.md) | direction/depth、graph capability、visited 去环、逐层 completion、branch isolation 和资源预算 |
...
| `src/sciretriever/normalization/`、`src/sciretriever/analysis/`、`src/sciretriever/packaging/` | [ADR 0003](../architecture/decisions/0003-operator-managed-mineru-service.md)、[系统设计](../architecture/system-design.md)、[技术架构](../architecture/technical-architecture.md)、[MinerU 注意事项](../notes/mineru.md) | 轻结构化文本边界、MinerU connection、PDF evidence、single current replacement 和 immutable package boundary；只有用户可获得的文本结果变化时同步需求 |
...
| `src/sciretriever/cli/`、配置模块、`docs/guides/config.toml`、`docs/guides/config.minimal.toml` | [README](../../README.md)、[用户手册](../guides/user-manual.md)、[配置手册](../guides/configuration.md)、[技术架构](../architecture/technical-architecture.md)、[AGENTS.md](../../AGENTS.md) | 已发布命令、参数、完整/最小模板职责、accepted keys/defaults/precedence、Provider 凭据、字段约束、offline/runtime 分界和 composition ownership；只有用户输入、核心流程或产品输出变化时同步需求 |
