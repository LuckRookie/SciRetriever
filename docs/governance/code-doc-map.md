...
| `src/sciretriever/integrations/graph.py`、引用解析与 expansion | [需求](../specs/requirements.md)、[系统设计](../specs/system-design.md)、[技术架构](../specs/technical-architecture.md)、[README](../../README.md)、[实施进度](implementation-progress.md) | direction/depth、graph provider capability、visited 去环、逐层 completion、branch isolation、资源预算与无隐藏文献总数上限 |
...
| `src/sciretriever/completion/`、`src/sciretriever/cli/completion_runtime.py` | [系统设计](../specs/system-design.md)、[技术架构](../specs/technical-architecture.md)、[README](../../README.md)、[AGENTS.md](../../AGENTS.md)、[实施进度](implementation-progress.md) | 四阶段事实派生、显式 stop、missing-suffix 调用、batch/interruption、force analysis、optional assets、单 invocation 装配和无第二套状态 |
...
| `src/sciretriever/diagnostics/`、`src/sciretriever/cli/failures.py`、统一计数 | [需求](../specs/requirements.md)、[系统设计](../specs/system-design.md)、[README](../../README.md)、[实施进度](implementation-progress.md) | 四阶段 failure 查询、latest/all、reason/action/rerun、显式脱敏 details、终态计数等式和 failure 不参与 completion facts |
| `src/sciretriever/cli/`、`src/sciretriever/config.py`、`src/sciretriever/config_loader.py`、`src/sciretriever/config_wp6.py`、`config.example.toml` | [README](../../README.md)、[需求](../specs/requirements.md)、[AGENTS.md](../../AGENTS.md)、[实施进度](implementation-progress.md) | 已发布命令、参数、stop/force/optional/import/expand/failures/config-check 语义、accepted keys/defaults/precedence、offline/runtime 分界、composition ownership，理想 CLI/config contract 和当前差距 |
