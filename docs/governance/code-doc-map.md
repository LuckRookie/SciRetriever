...
| `src/sciretriever/acquisition/`、`src/sciretriever/network/` | [系统设计](../specs/system-design.md)、[技术架构](../specs/technical-architecture.md)、[README](../../README.md)、[实施进度](implementation-progress.md) | provider、race、timeout、validation、理想回退顺序、已发布入口和当前缺口 |
| provider API、凭据生命周期、限流和现场排障 | [Provider 运维手册](../guides/provider-operations.md) | 公开契约、实测事实、证据等级、支持渠道知识和最后核对日期，不记录凭据值 |
