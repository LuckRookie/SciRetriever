...
class ExistingAssetImporter:
    ...
    def resolve_asset(path: str | Path, *, asset_root: str | Path | None = None) -> Path:
        resolved, _identity = ExistingAssetImporter._resolve_asset(
            path,
    ...
    def _snapshot(
        ...
        *,
        ...
        descriptor = self._open_without_symlinks(path)
        try:
            ...
            if expected_identity is not None and (
                ...
                raise ValueError("asset changed after path validation")
    ...
    def _open_without_symlinks(path: Path) -> int:
        ...
        try:
            ...
            return os.open(absolute.name, file_flags, dir_fd=directory)
        finally:
            os.close(directory)
    ...
    def import_asset(
        ...
        *,
        ...
        if replay is not None:
            return replay
        resolved, expected_identity = self._resolve_asset(
            path,
            asset_root=asset_root,
        )
        data = self._snapshot(resolved, expected_identity=expected_identity)
        ...
        if admission.reused_asset_id is not None:
            ...
            if admission.work_id is None:
                raise RuntimeError("admission is missing its work ID")
            return ExistingAssetImportResult(
                "replayed", admission.work_version_id, raw.id, raw.sha256, admission.work_id
            )
        if admission.job_id is None or not self.jobs.begin_existing_asset_import(admission.job_id):
        ...
        attempt = self.jobs.start_attempt(
            admission.job_id,
            "existing-asset-import",
            source_url="import://existing-asset",
        )
        try:
            accepted = self.coordinator.accept(
                BytesIO(data), admission.work_version_id, admission.job_id, role, media_type, format_name,
        ...
        try:
            ...
            if not (
                ...
                self.jobs.succeed_nonterminal_jobs_for_work(admission.work_version_id, role)
                raise
        except Exception:
            self.jobs.succeed_nonterminal_jobs_for_work(admission.work_version_id, role)
            raise
    ...
    def _authoritative_replay(
        ...
        version_ids = {work.preferred_work_version_id for work in works}
        if len(version_ids) != 1 or None in version_ids:
            return None
        work_version_id = next(iter(version_ids))
        assert work_version_id is not None
        link = next(
            (item for item in self.assets.get_work_version_assets(work_version_id) if item.asset_role is role),
            None,
        )
        if link is None:
            return None
        ...
        if raw is None:
            raise RuntimeError("authoritative reused asset is missing")
        return ExistingAssetImportResult("replayed", work_version_id, raw.id, raw.sha256, work_id)
