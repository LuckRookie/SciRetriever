...
def configure_parser(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--summary-max-characters", type=_positive, default=1_000)
    enrichment = parser.add_mutually_exclusive_group()
    enrichment.add_argument(
        "--enrichment",
        action="store_false",
        dest="no_enrichment",
        help="enable generic enrichment (default)",
    )
    enrichment.add_argument(
        "--no-enrichment",
        action="store_true",
        dest="no_enrichment",
        help="disable generic enrichment",
    )
    parser.set_defaults(no_enrichment=False)
...
def run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        result = PackagePipeline(
            ...
            summary_max_characters=args.summary_max_characters,
        ).run(
