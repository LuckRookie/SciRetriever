...
def prepare_initial_ingest(
    ...
    identifiers = tuple(
        sorted(
            {value for item in normalized for value in item.identifiers},
            key=lambda value: (value.namespace, value.value),
        )
    )
    ...
    if not identifiers and _metadata_signature(metadata) is None:
        evidence = "|".join(
            sorted(str(prepare_observation(item).observation_id) for item in normalized)
        )
    ...
    if selected is None and blocked_reasons:
        ...
        selected = next(
            (record for record in records if record.work_version_id == conflict_version_id), None
        )
    ...
    candidate_observations = tuple(
        ...
        for item in repository.list_observations(record.work_version_id)
    )
    ...
    revision = (
        1
        if selected is None or selected.metadata_revision is None
        else selected.metadata_revision + 1
    )
    ...
    snapshot = None
    if (selected is None or selected.current_metadata != metadata) and not (
        ...
        PreparedVersionRelation(
            VersionRelationId(
                str(
                    uuid5(
                        IDENTITY_NAMESPACE,
                        f"relation:{version_id}:{target.work_version_id}:{evidence.relation}",
                    )
                )
            ),
            version_id,
    ...
    for record in records:
        reasons = _identifier_conflicts(record.identifiers, identifiers) + _conflicts(
            record.current_metadata or InitialMetadata(), metadata
        )
        if record.work_version_id != version_id and (
