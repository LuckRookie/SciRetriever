...
from sciretriever.interoperability.library_details import CollectionMembershipPage, GraphPage, WorkDetail, WorkVersionDetail
...
class LibraryReadService:
    ...
    def get_work(self, work_id: WorkId, observations: bool = False, namespaces: tuple[str, ...] = ()) -> WorkDetail:
        return self._repository.get_work(work_id, observations, namespaces)

    def get_version(self, version_id: WorkVersionId, observations: bool = False, namespaces: tuple[str, ...] = ()) -> WorkVersionDetail:
        return self._repository.get_version(version_id, observations, namespaces)

    def references(self, version_id: str, unresolved: bool, limit: int, cursor: str | None) -> GraphPage:
