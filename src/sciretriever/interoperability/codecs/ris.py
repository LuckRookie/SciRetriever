        ...
        for ordinal, pairs in enumerate(groups):
            try:
                ...
                if "??" in fields or "TY" not in fields or "ER" not in fields:
                    raise CodecInputError("malformed-record", "RIS record has malformed or missing delimiters")
                one = lambda key: fields.get(key, [None])[0]
                year_text = one("PY") or one("Y1")
                date = one("DA")
                pages = "-".join(value for value in (one("SP"), one("EP")) if value)
                accession = one("AN")
                ids = tuple((key, value) for key, value in (("doi", one("DO")), ("pmid", accession), ("issn", one("SN"))) if value)
                notes = fields.get("N1", [])
                item_type = one("TY")
                result = record(
                    title=one("TI") or one("T1"), authors=fields.get("AU", []), identifier_values=ids,
                    abstract=one("AB"), keywords=fields.get("KW", []),
                    tags=(value[4:] for value in notes if value.casefold().startswith("tag:")),
                    references=fields.get("CR", []), institutions=fields.get("AD", []),
                    year=int(year_text[:4]) if year_text and year_text[:4].isdigit() else None,
                    month=int(date.split("/")[1]) if date and len(date.split("/")) > 1 and date.split("/")[1].isdigit() else None,
                    venue=one("JO") or one("JF") or one("T2"), volume=one("VL"), issue=one("IS"),
                    pages=pages, item_type=(
                        None if item_type is None else {"JOUR": "article", "BOOK": "book"}.get(item_type, item_type)
                    ),
                    language=one("LA"),
                )
