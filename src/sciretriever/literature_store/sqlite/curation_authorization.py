...
def authorize_plan_sources(  # noqa: C901
    connection: sqlite3.Connection,
    ...
    for item in plan.observation_moves:
        _require_owner(
            connection,
            "SELECT work_version_id FROM metadata_observations WHERE id=?",
    ...
    for item in plan.identifier_moves:
        _require_owner(
            connection,
            "SELECT work_version_id FROM stable_identifiers WHERE id=?",
    ...
    for item in plan.membership_moves:
        _require_owner(
            connection,
            "SELECT work_id FROM collection_memberships WHERE id=?",
    ...
    for item in plan.reference_retargets:
        ...
        if item.target_version_id is not None:
            ...
            planned_target = next(
                (
                    move.target_work_id
                    for move in plan.version_moves
                    if move.version_id == item.target_version_id
                ),
                None,
            )
            actual_target = None if row is None else WorkId(row[0])
            if item.target_work_id is None or item.target_work_id not in (
                actual_target,
                planned_target,
            ):
                raise sqlite3.IntegrityError(
                    "reference target WorkVersion does not belong to target Work"
                )
