...
from sciretriever.bibliography.api import (
    ArtifactRegistrationFact, CurationScope, CurationSnapshot, CurationTopology, IdentifierFact,
    MembershipFact, ObservationFact, ReferenceFact, RelationFact,
    IdentityCandidate, IdentityCandidateQuery,
    IdentityCandidateSet, IdentityRecord, SnapshotToken, StoredObservation,
    ValidatedVersionRelation, VersionFacts, WorkFacts,
    initial_metadata_from_json,
)
from sciretriever.kernel import AnalysisArtifactId, AssetId, LightDocumentId, MetadataSnapshotId, ObservationId, Sha256, UtcTimestamp, WorkId, WorkVersionId
from sciretriever.kernel.ids import MembershipId, ReferenceFactId, StableIdentifierId, VersionRelationId
...
def _work(connection, work_id: WorkId) -> WorkFacts | None:
    ...
    return WorkFacts(
        work_id, None if representative is None else WorkVersionId(representative[0]),
...
def _version(connection, version_id: WorkVersionId) -> VersionFacts | None:
    row = connection.execute(
        "SELECT v.work_id,v.version_role,m.metadata_snapshot_id,s.revision,s.sha256,p.work_version_asset_id,l.light_document_id,d.sha256,d.primary_asset_id,COALESCE(d.complete,0),b.light_document_id,b.analysis_artifact_id,a.light_document_id,a.input_sha256,COALESCE(a.nine_categories_complete,0),b.metadata_snapshot_id,b.reference_set_id,COALESCE(rs.complete,0),b.tag_set_id,COALESCE(ts.complete,0) FROM work_versions v LEFT JOIN work_version_current_metadata m ON m.work_version_id=v.id LEFT JOIN metadata_snapshots s ON s.id=m.metadata_snapshot_id LEFT JOIN accepted_primary_assets p ON p.work_version_id=v.id LEFT JOIN work_version_current_light_document l ON l.work_version_id=v.id LEFT JOIN light_documents d ON d.id=l.light_document_id AND d.work_version_id=v.id LEFT JOIN completion_bundles b ON b.work_version_id=v.id LEFT JOIN analysis_artifacts a ON a.id=b.analysis_artifact_id AND a.work_version_id=v.id LEFT JOIN reference_sets rs ON rs.id=b.reference_set_id AND rs.work_version_id=v.id LEFT JOIN tag_sets ts ON ts.id=b.tag_set_id AND ts.work_version_id=v.id WHERE v.id=?",
        (str(version_id),),
    ).fetchone()
    if row is None:
        return None
    return VersionFacts(
        WorkId(row[0]), version_id, row[1],
        None if row[2] is None else MetadataSnapshotId(row[2]), row[3],
...
class SqliteBibliographyRepository:
        ...
        with open_read_only_snapshot(self._catalog_path) as connection:
            for identifier in query.identifiers:
                rows = connection.execute(
                    "SELECT v.work_id,i.work_version_id FROM stable_identifiers i JOIN work_versions v ON v.id=i.work_version_id WHERE i.namespace=? AND i.value=?",
    ...
    def load_curation_topology(self) -> CurationTopology:
        with open_read_only_snapshot(self._catalog_path) as connection:
            work_ids = tuple(WorkId(row[0]) for row in connection.execute("SELECT id FROM works ORDER BY id"))
            version_ids = tuple(WorkVersionId(row[0]) for row in connection.execute("SELECT id FROM work_versions ORDER BY id"))
            ...
            versions = tuple(version_values)
            identifiers = tuple(
                IdentifierFact(StableIdentifierId(row[0]), WorkVersionId(row[1]), Identifier(row[2], row[3]))
                for row in connection.execute("SELECT id,work_version_id,namespace,value FROM stable_identifiers ORDER BY id")
            )
            observations = tuple(
                ObservationFact(ObservationId(row[0]), WorkVersionId(row[1]))
                for row in connection.execute("SELECT id,work_version_id FROM metadata_observations ORDER BY id")
            )
            memberships = tuple(
                MembershipFact(MembershipId(row[0]), CollectionId(row[1]), WorkId(row[2]))
                for row in connection.execute("SELECT id,collection_id,work_id FROM collection_memberships ORDER BY id")
            )
            references = tuple(
                ReferenceFact(
                    ReferenceFactId(row[0]), WorkVersionId(row[1]),
                ...
                for row in connection.execute(
                    ...
                    "COALESCE(json_extract(m.reference_json,'$.raw_text'),m.reference_json),m.reference_json "
                    "FROM reference_members m JOIN reference_sets s ON s.id=m.reference_set_id ORDER BY m.id"
                )
            )
            relations = tuple(
                RelationFact(VersionRelationId(row[0]), WorkVersionId(row[1]), WorkVersionId(row[2]), row[3])
                for row in connection.execute("SELECT id,left_version_id,right_version_id,relation FROM work_version_relations ORDER BY id")
            )
            registrations = tuple(
            ...
            return CurationTopology(snapshot, identifiers, observations, memberships, references, relations, registrations)
    ...
    def list_identity_records(self) -> tuple[IdentityRecord, ...]:
        with open_read_only_snapshot(self._catalog_path) as connection:
            rows = connection.execute(
                "SELECT v.work_id,v.id,v.version_role,r.work_version_id,s.values_json,s.revision,b.work_version_id "
            ...
            for work_id, version_id, role, representative_id, values_json, revision, completed_id in rows:
                ...
                identifiers = tuple(Identifier(namespace, value) for namespace, value in identifier_rows)
                revision_payload = "|".join((work_id, version_id, role, *(f"{item.namespace}:{item.value}" for item in identifiers), values_json or ""))
                result.append(IdentityRecord(
                    WorkId(work_id), WorkVersionId(version_id),
                    ...
                    revision, completed_id is not None,
    ...
    def list_observations(self, version_id: WorkVersionId) -> tuple[StoredObservation, ...]:
        ...
        return tuple(
            StoredObservation(
                ObservationId(identifier), provider, provider_record_id,
                Sha256(payload_sha256), payload_json, UtcTimestamp(observed_at),
            )
            for identifier, provider, provider_record_id, payload_sha256, payload_json, observed_at in rows
        )
