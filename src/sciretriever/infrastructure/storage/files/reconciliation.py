...
from sciretriever.infrastructure.storage.files.artifact_identity import (
    FormalArtifactCandidate,
    descriptor_identity,
    formal_artifact_path,
    open_immutable_artifact,
)
from sciretriever.infrastructure.storage.files.artifacts import (
...
from sciretriever.services.library.ports import ArtifactReferenceReader
...
class ReconciliationConfig:
    storage_root: Path
    reference_reader: ArtifactReferenceReader
...
class CoreArtifactReconciler:
    ...
    def _reconcile(self, checkpoint: Checkpoint | None) -> ReconciliationResult:
        ...
        with CoreStorage(config.storage_root).open_core() as core:
            ...
            referenced = set(config.reference_reader.referenced_artifact_paths())
