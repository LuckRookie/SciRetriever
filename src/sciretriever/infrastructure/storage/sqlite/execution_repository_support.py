...
_STORED_RESULT_ERROR = "stored target result must be a valid TargetResultEnvelope"
...
def _stored_result(payload: str | None) -> TargetResult | None:
    ...
    try:
        ...
        if not isinstance(value, CanonicalJsonObject):
            raise CanonicalJsonError("stored target result envelope must be a JSON object")
        details = _object_item(value, "details")
        if not isinstance(details, CanonicalJsonObject):
            raise CanonicalJsonError("stored target result envelope details must be a JSON object")
        envelope = TargetResultEnvelope.model_validate_json(canonical_json_bytes(value))
    except (CanonicalJsonError, ValidationError) as error:
        raise ExecutionPersistenceError(_STORED_RESULT_ERROR) from error
