# noqa: E501  # noqa: SIZE_OK - completion facts and target result share one atomic transaction
...
class CompletionPublisher:
    ...
    def publish_completion(  # noqa: C901
        self,
        acceptance: ValidatedCompletionAcceptance,
    ) -> CompletionOutcome:
        submission = acceptance.submission
        target = acceptance.target_projection
        ...
        result_json = target_result_envelope_json(acceptance.target_result)
        identity_sha256 = acceptance.identity_sha256
        point = StatementFailpoint(self._failpoint)
        with immediate(self._catalog_path) as connection:
            target_row = connection.execute(
                "SELECT b.status,t.result_json FROM batch_runs b JOIN batch_targets t ON "
                "t.batch_run_id=b.id AND t.target_kind='work-version' AND t.target_id=? WHERE "
                "b.id=? AND b.batch_type='process'",
                (str(submission.work_version_id), str(target.batch_run_id)),
            ).fetchone()
            if target_row is None:
                raise StalePublicationError("completion target is not a process target")
            ...
            if existing is not None:
                if target_row[1] == result_json and existing == expected_bundle:
                    return CompletionOutcome.REPLAYED
                raise StalePublicationError("work version already has a divergent completion")
            if target_row[0] != "running":
                raise StalePublicationError("completion target is not running")
