"""Model capabilities and local readiness, independent of provider names."""

from __future__ import annotations

from dataclasses import dataclass

from .requests import AgentCapability, AgentRole
...
class AgentModelCapabilities:
    ...
    def __post_init__(self) -> None:
        ...
        _validate_token_limits(self.context_window_tokens, self.max_output_tokens)
...
class AgentReadiness:
    analysis: AgentCapabilityReadiness
    browser: AgentCapabilityReadiness

    def __post_init__(self) -> None:
        if not isinstance(self.analysis, AgentCapabilityReadiness):
            raise TypeError("analysis readiness is invalid")
        if not isinstance(self.browser, AgentCapabilityReadiness):
            raise TypeError("browser readiness is invalid")
        if self.analysis.role is not AgentRole.ANALYSIS:
            raise ValueError("analysis readiness must describe the analysis role")
        if self.browser.role is not AgentRole.BROWSER:
            raise ValueError("browser readiness must describe the browser role")
    ...
    return frozenset(
        {
            ...
            AgentCapability.TOOL_DECISION,
        }
    )


def _validate_token_limits(context_window_tokens: int, max_output_tokens: int) -> None:
