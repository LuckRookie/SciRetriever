...
def find_workspace_root(start: Path | None = None) -> Path:
    ...
    override = os.environ.get("SCIRETRIEVER_WORKSPACE_ROOT")
    if override:
        ...
        if not workspace_root.is_dir() or not _has_valid_workspace_marker(workspace_root):
            raise RuntimeError("SCIRETRIEVER_WORKSPACE_ROOT must contain a valid .workspace-root marker.")
    ...
    for candidate in (current, *current.parents):
        if _has_valid_workspace_marker(candidate):
            ...
            return candidate
    raise RuntimeError(
        "Unable to find the workspace root containing a valid .workspace-root marker; "
        "set SCIRETRIEVER_WORKSPACE_ROOT or pass an explicit CLI path."
    )
