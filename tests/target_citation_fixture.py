...
from sciretriever.collection.api import (
    CitationDiscoveryRequest, CitationObservation, ProviderCitationResult,
)
...
from sciretriever.collection.api import ProviderDiscoveryResult, TopicConditions
...
from sciretriever.literature_store.sqlite import (
    ...
    SqliteCollectionRepository, create_or_open_catalog,
)
    ...
    def expand(self, request: CitationDiscoveryRequest) -> ProviderCitationResult:
        ...
        return ProviderCitationResult(
            ...
            tuple(CitationObservation(
                self.provider, request.seed, item, request.direction,
            ) for item in values),
            None,
        )
...
class CitationCollectionTestCase(unittest.TestCase):
    ...
    def service(self, citation_sources: tuple[CitationSource, ...] = ()) -> CollectionService:
        ...
        return CollectionService(CollectionServiceDependencies(
            self.collections, self.bibliography, CollectionAcceptancePublisher(self.catalog),
            lambda: self.bound.port.acquire_core_write(self.bound.identity),
            (MetadataSource("seed", metadata),), citation_sources,
        ))

    def add_work(self, doi: str):
        metadata = FakeMetadataPort(
            self.catalog, ProviderDiscoveryResult(
                "seed", (observation("seed", doi, doi),), None,
            ), [],
        )
        ...
        record = next(
            item for item in repository.list_identity_records()
