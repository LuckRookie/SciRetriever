...
from sciretriever.interoperability.codecs import (
    BibtexCodec, CslJsonCodec, EndnoteXmlCodec, RisCodec,
)
from sciretriever.interoperability.import_preparation import (
    ImportIdentityResolution, ImportPreparationRequest, prepare_import_record,
)
...
BIBTEX = b"""@article{x,title={Canonical Title},author={Lovelace, Ada and Hopper, Grace},abstract={Summary},year={2026},month={7},journal={Journal},volume={4},number={2},pages={10--20},doi={https://doi.org/10.1000/Example},pmid={42},issn={1234-5678},language={en},keywords={alpha; beta; alpha},tags={review; core},references={First ref; Second ref},file={/private/paper.pdf},note={secret}}"""
RIS = b"""TY  - JOUR\nTI  - Canonical Title\nAU  - Lovelace, Ada\nAU  - Hopper, Grace\nAB  - Summary\nPY  - 2026\nDA  - 2026/07\nJO  - Journal\nVL  - 4\nIS  - 2\nSP  - 10\nEP  - 20\nDO  - 10.1000/Example\nAN  - PMID:42\nSN  - 1234-5678\nLA  - en\nKW  - alpha\nKW  - beta\nKW  - alpha\nN1  - tag:review\nN1  - tag:core\nCR  - First ref\nCR  - Second ref\nL1  - /private/paper.pdf\nER  - \n"""
CSL = b'''[{"id":"private-id","type":"article-journal","title":"Canonical Title","author":[{"family":"Lovelace","given":"Ada"},{"family":"Hopper","given":"Grace"}],"abstract":"Summary","issued":{"date-parts":[[2026,7]]},"container-title":"Journal","volume":"4","issue":"2","page":"10-20","DOI":"10.1000/Example","PMID":"42","ISSN":"1234-5678","language":"en","keyword":"alpha; beta; alpha","categories":["review","core"],"references":["First ref","Second ref"],"note":"secret","attachments":["/private/paper.pdf"]}]'''
ENDNOTE = b'''<?xml version="1.0" encoding="UTF-8"?><xml><records><record><ref-type name="Journal Article">17</ref-type><titles><title>Canonical Title</title><secondary-title>Journal</secondary-title></titles><contributors><authors><author>Lovelace, Ada</author><author>Hopper, Grace</author></authors></contributors><abstract>Summary</abstract><dates><year>2026</year><month>7</month></dates><volume>4</volume><number>2</number><pages>10-20</pages><electronic-resource-num>10.1000/Example</electronic-resource-num><accession-num>PMID:42</accession-num><isbn>1234-5678</isbn><language>en</language><keywords><keyword>alpha</keyword><keyword>beta</keyword><keyword>alpha</keyword></keywords><custom1>review</custom1><custom2>core</custom2><references><reference>First ref</reference><reference>Second ref</reference></references><notes>secret</notes><urls><pdf-urls><url>/private/paper.pdf</url></pdf-urls></urls></record></records></xml>'''
...
class TargetBibliographyImportTests(unittest.TestCase):
    def test_four_formats_converge_without_private_fields(self) -> None:
        codecs_and_bytes = (
            (BibtexCodec(), BIBTEX), (RisCodec(), RIS),
            (CslJsonCodec(), CSL), (EndnoteXmlCodec(), ENDNOTE),
        )

        records = tuple(codec.read(BytesIO(payload))[0].record for codec, payload in codecs_and_bytes)
    ...
    def test_endnote_rejects_utf16_declarations_before_entity_expansion(self) -> None:
        ...
        results = tuple(
            EndnoteXmlCodec().read(BytesIO(
                (b"\xff\xfe" if encoding.endswith("le") else b"\xfe\xff")
                + (f'<?xml version="1.0" encoding="UTF-16"?>{declaration}'
                   '<xml><records><record><titles><title>&x;</title></titles></record></records></xml>').encode(encoding)
            ))[0]
            for declaration in declarations for encoding in codecs
        )
    ...
    def test_endnote_namespace_variants_map_identically_and_reject_unknown_namespace(self) -> None:
        body = '<records><record><titles><title>Namespaced</title></titles><references><reference>R</reference></references></record></records>'
        default = f'<xml xmlns="urn:endnote">{body}</xml>'.encode()
        prefixed = f'<e:xml xmlns:e="urn:endnote">{body.replace("<", "<e:").replace("<e:/", "</e:")}</e:xml>'.encode()
        unknown = f'<xml xmlns="urn:unknown">{body}</xml>'.encode()
        collision = b'<xml><record><title>Wrong</title></record><records><record><titles><title>Namespaced</title></titles></record></records></xml>'
    ...
    def test_endnote_encoding_contract_rejects_mismatch_and_nul_but_accepts_benign_utf16(self) -> None:
        benign = '<?xml version="1.0" encoding="UTF-16"?><xml><records><record><titles><title>Benign</title></titles></record></records></xml>'
        declared_le = benign.replace('encoding="UTF-16"', 'encoding="UTF-16LE"').encode("utf-16-le")
        mismatched_bom = b"\xff\xfe" + benign.replace('encoding="UTF-16"', 'encoding="UTF-16BE"').encode("utf-16-le")
        ...
        good = tuple(EndnoteXmlCodec().read(BytesIO(value))[0] for value in (
            b"\xff\xfe" + benign.encode("utf-16-le"),
            b"\xfe\xff" + benign.encode("utf-16-be"), declared_le,
        ))
        bad = tuple(EndnoteXmlCodec().read(BytesIO(value))[0] for value in (
            mismatched_bom, mismatch, unsupported, nul,
        ))
    ...
    def test_replay_and_mixed_format_use_identity_result_without_writes(self) -> None:
        identity = FakeIdentity(ImportIdentityResolution(
            WorkId("00000000-0000-0000-0000-000000000001"),
            WorkVersionId("00000000-0000-0000-0000-000000000002"),
            ImportResult.DUPLICATE, False, None,
        ))
        parsed = (
