...
class NormalizationServiceTests(TestCase):
    def setUp(self) -> None:
        ...
        self.work_version_id = IdentityResolver(self.catalog).create_or_reuse_work({"doi": "10.1/service"}).work_version.id
        ...
        with self.catalog.transaction() as connection:
            ...
            connection.exec_driver_sql("INSERT INTO work_version_assets (work_version_id, raw_asset_id, asset_role) VALUES (?, ?, ?)", (self.work_version_id, self.raw_id, "xml"))
    ...
    def test_publication_registration_and_replay_without_parsing(self) -> None:
        ...
        first = service.run(self.work_version_id)
        ...
        with patch("sciretriever.normalization.service.normalize_inputs", side_effect=AssertionError("reparsed")):
            replay = service.run(self.work_version_id)
    ...
    def test_preexisting_active_run_is_recovered(self) -> None:
        ...
        result = service.run(self.work_version_id)
