...
from sciretriever.catalog import CitationRepository, EnrichmentRepository, IdentityResolver, apply_migrations, create_catalog_engine
...
class EnrichmentRepositoryTests(TestCase):
    def setUp(self) -> None:
        ...
        self.work_id = resolver.create_or_reuse_work({"doi": "10.1/citing"}).work.id
        ...
        with self.catalog.transaction() as connection:
            ...
            connection.exec_driver_sql("INSERT INTO normalized_artifacts (id, work_id, raw_asset_id, kind, schema_version, storage_path, sha256, media_type, byte_size, provenance_json) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (self.artifact_id, self.work_id, raw_id, "normalized_content", "1", "derived/normalized/item", "b" * 64, "application/json", 10, "{}"))

    def test_light_results_and_citations_replay(self) -> None:
        enrichment = EnrichmentRepository(self.catalog)
        summary = enrichment.register_result(self.work_id, self.artifact_id, "summary", "1", "c" * 64, {"summary": "Text"})
        self.assertEqual(enrichment.register_result(self.work_id, self.artifact_id, "summary", "1", "c" * 64, {"summary": "Text"}), summary)
        citations = CitationRepository(self.catalog).register_identifier_links(
            self.work_id, self.artifact_id, (Identifier("doi", "10.1/cited"), Identifier("pmid", "999"))
        )
        ...
        replay = CitationRepository(self.catalog).register_identifier_links(
            self.work_id, self.artifact_id, (Identifier("pmid", "999"), Identifier("doi", "10.1/cited"))
        )
