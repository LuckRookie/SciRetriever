...
from sciretriever.collection.api import (
    CausePageRequest, CitationCollectionRequest, CitationDiscoveryRequest, CitationRunInput,
    CollectionSeed, IdentifierSeed, PathPageRequest, WorkSeed,
    WorkVersionSeed, TopicConditions,
)
...
from sciretriever.kernel import (
    BoundaryError, CitationDirection, CollectionId, Identifier, WorkId, WorkVersionId,
    WorkVersionState,
)
from sciretriever.literature_store.sqlite import (
    CollectionAcceptancePublisher, open_read_only_snapshot,
)
from test_target_collection import FakeMetadataPort
from sciretriever.collection.api import ProviderDiscoveryResult
from sciretriever.bibliography.api import IdentityCandidate, IdentityCandidateQuery, IdentityCandidateSet
from target_citation_fixture import (
    CitationCollectionTestCase, FakeCitationPort, FixedCandidateBibliography,
)
...
class TargetCitationCollectionTests(CitationCollectionTestCase):
    ...
    def test_four_seed_kinds_resolve_dedupe_sort_and_persist_canonical_input(self) -> None:
        ...
        request = CitationCollectionRequest(
            (
                WorkSeed(second.work_id), WorkVersionSeed(first.work_version_id),
        ...
        sources = tuple(CitationSource(
            name, FakeCitationPort(self.catalog, {}, name, requests),
        ) for name in ("alpha", "beta"))
        result = self.service(sources).run_citation(
            target.collection_id, request, WorkVersionState.LIGHT_TEXT_READY,
        )
        ...
        self.assertEqual((persisted.providers, persisted.direction, persisted.depth, persisted.max_new),
                         (("alpha", "beta"), CitationDirection.BOTH, 0, 9))
    ...
    def test_invalid_seed_rejects_before_run_creation(self) -> None:
        ...
        selectors = (
            WorkSeed(WorkId(missing)), WorkVersionSeed(WorkVersionId(missing)),
        ...
        for selector in selectors:
            with self.subTest(selector=selector), self.assertRaises(BoundaryError):
                self.service((source,)).run_citation(
                    target.collection_id, CitationCollectionRequest(
                        (selector,), ("alpha",), CitationDirection.REFERENCES, 1, 10,
    ...
    def test_ambiguous_identifier_rejects_before_run_creation(self) -> None:
        ...
        bibliography = FixedCandidateBibliography(self.bibliography, IdentityCandidateSet((
        ...
        service = CollectionService(CollectionServiceDependencies(
            self.collections, bibliography, CollectionAcceptancePublisher(self.catalog),
            lambda: self.bound.port.acquire_core_write(self.bound.identity),
            (MetadataSource("seed", FakeMetadataPort(
        ...
        with self.assertRaises(BoundaryError):
            service.run_citation(target.collection_id, CitationCollectionRequest(
                (IdentifierSeed(identifier),), ("alpha",), CitationDirection.REFERENCES, 1, 10,
            ), WorkVersionState.UNREVIEWED)
    ...
    def test_cyclic_layers_keep_duplicate_paths_isolate_provider_failure_and_restart(self) -> None:
        ...
        alpha_edges = {
            (str(first.work_id), CitationDirection.REFERENCES): (
                identifiers[str(third.work_id)], identifiers[str(second.work_id)],
                identifiers[str(second.work_id)],
            ),
            (str(second.work_id), CitationDirection.REFERENCES): (
                identifiers[str(first.work_id)], identifiers[str(fourth.work_id)],
            ),
        ...
        sources = (
            ...
            CitationSource("beta", FakeCitationPort(
                self.catalog, beta_edges, "beta", calls, frozenset((str(second.work_id),)),
            )),
        )
        service = self.service(sources)
        target = service.create("cyclic", None, None)
        request = CitationCollectionRequest(
            (WorkSeed(first.work_id),), ("alpha", "beta"),
            CitationDirection.REFERENCES, 2, 10,
        )

        first_run = service.run_citation(
            target.collection_id, request, WorkVersionState.COMPLETED,
        )
        ...
        causes = self.collections.list_causes(CausePageRequest(target.collection_id, None, 100)).causes
    ...
    def test_max_new_and_no_new_are_distinct_durable_stops(self) -> None:
        ...
        edges = {(str(seed.work_id), CitationDirection.REFERENCES): (
            Identifier("doi", "10.1/limit-b"), Identifier("doi", "10.1/limit-a"),
        )}
        calls: list[CitationDiscoveryRequest] = []
        ...
        max_run = service.run_citation(limited.collection_id, CitationCollectionRequest(
            (WorkSeed(seed.work_id),), ("alpha",), CitationDirection.REFERENCES, 4, 1,
        ), WorkVersionState.UNREVIEWED)
        no_new_run = service.run_citation(empty.collection_id, CitationCollectionRequest(
            (WorkSeed(second.work_id),), ("alpha",), CitationDirection.REFERENCES, 4, 3,
        ), WorkVersionState.UNREVIEWED)
