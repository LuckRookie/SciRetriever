...
from sciretriever.catalog import IdentityResolver, ProcessingRunRepository, apply_migrations, create_catalog_engine
...
class ProcessingRepositoryTests(TestCase):
    def setUp(self) -> None:
        ...
        self.work_id = IdentityResolver(self.catalog).create_or_reuse_work({"doi": "10.1/processing"}).work.id
    ...
    def test_claim_success_and_exact_replay_are_idempotent(self) -> None:
        first = self.repository.claim_or_resume(
            self.work_id, ProcessingStage.NORMALIZATION, "normalizer", "1", {"pages": 10}
        )
        replay = self.repository.claim_or_resume(
            self.work_id, "normalization", "normalizer", "1", {"pages": 10}
        )
    ...
    def test_failure_is_recorded_once_and_can_resume(self) -> None:
        run = self.repository.claim_or_resume(
            self.work_id, "enrichment", "enricher", "1", {"summary": 200}
        )
