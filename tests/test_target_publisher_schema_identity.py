...
from sciretriever.literature_store.sqlite import (
    UnsupportedCatalogError, create_or_open_catalog, validate_catalog,
)
...
class TargetPublisherSchemaIdentityTests(unittest.TestCase):
    ...
    def test_supported_raw_writes_reject_false_canonical_hash_identity(self) -> None:
        with create_or_open_catalog(self.catalog) as connection:
            ...
            connection.execute(
                "INSERT INTO artifacts(id,kind,sha256,storage_path,byte_size) VALUES ('raw','raw',?,'primary/aa/value',1)",
                ("a" * 64,),
            )
            connection.execute("INSERT INTO raw_assets VALUES ('raw','primary-pdf','{}')")
            connection.execute("INSERT INTO work_version_assets VALUES ('primary','version','raw','primary-pdf')")
            connection.execute("INSERT INTO accepted_primary_assets VALUES ('version','primary')")
            connection.execute(
                "INSERT INTO artifacts(id,kind,sha256,storage_path,byte_size) VALUES ('light-artifact','light-document',?,'light-document/44/value',2)",
                (canonical_digest,),
            )
            with self.assertRaises(sqlite3.IntegrityError):
                connection.execute(
                    "INSERT INTO light_documents VALUES ('light','version','primary','light-artifact',?,'{}','{}',1)",
                    (false_digest,),
                )
            ...
            connection.execute(
                "INSERT INTO artifacts(id,kind,sha256,storage_path,byte_size) VALUES ('wrong-kind','raw',?,'primary/44/wrong',2)",
                (canonical_digest,),
            )
            with self.assertRaises(sqlite3.IntegrityError):
                connection.execute(
                    "INSERT INTO light_documents VALUES ('light-wrong-kind','version','primary','wrong-kind',?,'{}','{}',1)",
                    (canonical_digest,),
                )
            ...
            connection.execute(
                "INSERT INTO artifacts(id,kind,sha256,storage_path,byte_size) VALUES ('analysis-artifact','analysis',?,'analysis/44/value',2)",
                (canonical_digest,),
            )
            with self.assertRaises(sqlite3.IntegrityError):
                connection.execute(
                    "INSERT INTO analysis_artifacts VALUES ('analysis','version','light','analysis-artifact',?,?, '{}','{}',1)",
    ...
    def test_catalog_validation_rejects_rows_forged_with_false_hash_udf(self) -> None:
        ...
        with sqlite3.connect(self.catalog) as connection:
            connection.create_function(
                "sciretriever_canonical_json", 1, lambda payload: payload,
                deterministic=True,
            )
            connection.create_function(
